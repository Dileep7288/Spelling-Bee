...
class GameViewController: UIViewController,UITextFieldDelegate {
    ...
    private func showSpeakerPopup() {
        ...
        let customAlert = CustomAlertView(
            title: "How to Play?",
            message: "Click the Speaker icon button to listen to the word. Once you hear it, type the word into the input box below. Good luck!",
            buttonTitle: "PROCEED"
        )
    ...
    private func fetchSpellBeeData() {
        guard let url = URL(string: APIConstants.spellBeeEndpoint) else { return }
        ...
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            ...
            do {
                let result = try JSONDecoder().decode(SpellBeeData.self, from: data)
                DispatchQueue.main.async {
                    self.words = result.words
                    self.sentences = result.sentences
                    self.currentIndex = 0
                    self.submitDate = result.date
    ...
    //MARK: Submit Game
    func submitGameResult(completion: @escaping (Bool) -> Void) {
        let urlString = APIConstants.submitGame
        ...
        guard let submitDate = self.submitDate, !submitDate.isEmpty else {
            print("❌ Missing submitDate")
            completion(false)
            return
        }
        ...
        let gameData: [String: Any] = [
            "rider_id": 102,
            "event_id": "station-s",
            "mistake_words": wrongWords,
            "correct_words": correctWords,
            "total_points": score,
            "submit_date": submitDate
        ]

        guard let jsonData = try? JSONSerialization.data(withJSONObject: gameData, options: []) else {
    ...
    private func showExitConfirmationPopup() {
        inputTextField.resignFirstResponder()
        pauseTimer()

        let customAlert = CustomAlertVC()
        customAlert.onCancelTapped = {
            self.resumeTimer()
            self.inputTextField.becomeFirstResponder()
        }
        customAlert.onYesTapped = {
            self.submitZeroScoreAndNavigateToMainScreen() 
        }

        if let window = UIApplication.shared.keyWindow {
            customAlert.show(in: window)
        }
    }
    ...
    //MARK: Submit Zero Values
    private func submitZeroScoreAndNavigateToMainScreen() {
        let gameData: [String: Any] = [
            "rider_id": 101,
            "event_id": "station-s",
            "mistake_words": 0,
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                if let error = error {
                    ...
                    print("✅ Successfully submitted zero score")
                } else {
                    print("❌ Server error: \(response.debugDescription)")
                }
